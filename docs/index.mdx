...
**Wasmbox** is full [WebAssembly](https://webassembly.org/) runtime for Unity, based on [Wasmtime](https://wasmtime.dev/). WebAssembly (WASM) makes it possible to execute compiled code (e.g. C, C++ or Rust) in a safe and secure sandbox - providing high **performance**, **safe execution** of malicious/buggy code, cross platform **portability** and **deterministic** execution.
...
WebAssembly files (WASM/WAT format) are imported through the standard Unity asset pipeline. Automatic code generation writes much of the complex "interop" code. This includes support for the Unity [Job/Safety System](https://docs.unity3d.com/Manual/JobSystem.html), allowing WASM code to be executed off the main thread. Wasmbox includes a `MonoBehaviour` based script which handles all of the complex work of loading and invoking WASM for you.

## How Can I Buy It?

<b>Buy Wasmbox from the <links.AssetStore />!</b>

...
See the [Importing WebAssembly](./../reference/editor/import.md) documentation for more detail on import settings.
...
Create a new Unity script which uses [`SimpleWasmMonoBehaviour<>`](./../reference/code/simplewasmmonobehaviour.md) instead of `MonoBehaviour`. Inside the angle brackets put the name of the generated C# script.
...
The `Engine Config` field controls how the WASM code will be compiled and executed. Including [Fuel](./fuelusage.md) and [Epoch Interruption](./epochinterruption.md) to limit buggy/malicious code execution.
